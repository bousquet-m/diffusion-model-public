...
def coordination(frames: list[Atoms], z_center: int, z_neighbor: int,
    ...
    for atoms in frames:
        ...
        np.add.at(counts, i_sel, 1)
        cns.extend(counts[num == z_center].tolist())
...
def summary(frames: list[Atoms], z_center: int = 49, z_neighbor: int = 8,
            cutoff: float = 2.7) -> dict:
    cns, bonds = coordination(frames, z_center, z_neighbor, cutoff)
    return {
